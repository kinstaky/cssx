...
public:
	...
	void Set(T1 key, T2 val) {
		for (typename multimap<T1, T2, Pred>::iterator iter = this->lower_bound(key); iter != this->upper_bound(key); ++iter) iter->second = val;
		return;
	}
};

template <class T1, class T2>
ostream & operator<<(ostream &os, pair<const T1, T2> p) {
	os << "(" << p.first << "," << p.second << ")";
	return os;
}
